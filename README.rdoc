This is alpha-alpha-pre-release software. A lot of things will change before we reach 0.1.
...
== Highlights
  * Animation(), Sprite(), SpriteSheet() and other constructors
  ...
  * QUnit testsuite @ http://ippa.se/jaws/test/
...
== Examples
* http://ippa.se/jaws/examples/example1.html
* http://ippa.se/jaws/examples/example2.html
* http://ippa.se/jaws/examples/example3.html
...
== Example Code
  ...
  <script>
    ...
    jaws.start()  // call setup(), then loop calling update() / game.draw() with FPS 60
  </script>
...
== jaws.Sprite() - simple movable image-based object on screen
  ...
  sprite = new Sprite({image: "player.png", x: 10, y: 10, scale: 4})
  sprite.draw()


== jaws.SpriteSheet() - cut big sprite sheets into smaller images from left to right, top to bottom
  spriteSheet = new SpriteSheet({image: "droid_11x15.bmp", frameSize => [32,32]})
