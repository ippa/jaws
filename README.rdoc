...
== Learn more
* http://ippa.se/jaws          - Official Jaws homepage. Jaws docs and design choices.
* http://ippa.se/jaws/test/    - Jaws QUnit testsuite

== Examples online
Check out the sourcecode for comments and explanations:

* http://ippa.se/jaws/examples/example1.html  - focus: manual setup of canvas. later examples shows better ways.
* http://ippa.se/jaws/examples/example2.html  - focus: jaws game states
* http://ippa.se/jaws/examples/example3.html  - focus: Animation()
...
== Code demo
  ...
  //
  // Below code shows the way I like, letting jaws worry about most of the boring setup stuff
  // so I can get straight to get game logic.
  //
  <html>
  ...
  <script>
    ...
    function MyGameState() {
      ...
      this.setup = function() {
        ...
        player = new Sprite({image: "player.png", x: 10, y: 200})
        
        // Let's create an animated robot sprite
        robot = new Sprite({x: 200, y: 200})
        robot.sheet = new SpriteSheet({image: "droid_11x15.png", frameSize => [11,15]})
        robot.animation = Animation.new({frames: robot_sheet.frames, frame_duration: 120})
      }
